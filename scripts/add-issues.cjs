...
const newIssues = [
  ...
  },
  {
    ...
    fix: [
      ...
      'promo_codes.practitioner_id: ON DELETE RESTRICT (confirm with business)'
    ],
    requirements: [
  ...
  },
  {
    ...
    fix: [
      'In alerts.css: replace rgba magic values with var(--color-*) token equivalents',
      'In calendar.css: replace #fff, #ff6b6b, #6C63FF, #FFD93D, rgba magic values with token references',
      'Add alpha-channel token variants to tokens.css if not present',
      'Grep for remaining hard-coded hex/rgba values across all CSS component files after fix'
    ],
    requirements: [
      'Every color in a component CSS file must reference a var(--token) from tokens.css',
      'Exceptions allowed for transparent, currentColor, inherit only'
    ],
    acceptanceCriteria: [
      'alerts.css and calendar.css contain zero hex or raw rgba() color values',
