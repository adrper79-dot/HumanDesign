...
// E2E bypass: ?e2e=1 marks first-run modal as seen so tests can proceed without dismissing it.
// Only affects the modal guard — all other auth and routing behaviour is unchanged.
...
// ── Billing / Pricing — extracted to controllers/billing-controller.js ──────
...
function switchTab(id, btn) {
  ...
  // Auto-load clusters when tab is activated (clustering-controller.js loaded lazily)
  if (id === 'clusters' && token && !document.getElementById('clusterListContainer')?.innerHTML && !switchTab._loading.has('clusters')) {
    ...
    _loadController('clustering-controller.js')
      .then(() => loadClusters())
      .catch(e => console.error(e))
      .finally(() => switchTab._loading.delete('clusters'));
  }

  // Auto-load practitioner roster when tab is activated (4 practitioner controllers loaded lazily)
  if (id === 'practitioner' && token && !switchTab._loading.has('practitioner')) {
    switchTab._loading.add('practitioner');
    Promise.all([
      _loadController('practitioner-clients.js'),
      _loadController('practitioner-marketing.js'),
      _loadController('practitioner-management.js'),
      _loadController('practitioner-notes.js'),
    ]).then(() => loadRoster())
      .catch(e => console.error(e))
      .finally(() => switchTab._loading.delete('practitioner'));
  }

  // Auto-load client portal when tab is activated (practitioner-clients.js loaded lazily)
  if (id === 'my-practitioner' && token && !switchTab._loading.has('my-practitioner')) {
    switchTab._loading.add('my-practitioner');
    _loadController('practitioner-clients.js')
      .then(() => loadClientPortal())
      .catch(e => console.error(e))
      .finally(() => switchTab._loading.delete('my-practitioner'));
  }

  // Auto-load celebrity matches on first visit (achievements-controller.js loaded lazily)
  if (id === 'celebrity' && token && !document.getElementById('celebrityGrid')?.innerHTML && !switchTab._loading.has('celebrity')) {
    ...
    _loadController('achievements-controller.js')
      .then(() => loadCelebrityMatches())
      .catch(e => console.error(e))
      .finally(() => switchTab._loading.delete('celebrity'));
  }

  // Auto-load achievements on first visit (achievements-controller.js loaded lazily)
  if (id === 'achievements' && token && !document.getElementById('achievementsBadges')?.innerHTML && !switchTab._loading.has('achievements')) {
    ...
    _loadController('achievements-controller.js')
      .then(() => {
        if (!document.getElementById('achievementsBadges')?.innerHTML) loadAchievements();
        if (!document.getElementById('leaderboardList')?.innerHTML) loadLeaderboard();
      })
      .catch(e => console.error(e))
      .finally(() => switchTab._loading.delete('achievements'));
  }
  ...
  // UX-008: leaderboard now loaded together with loadAchievements() in the achievements block above
...
// ── Practitioner Clients — extracted to controllers/practitioner-clients.js ──
// ── Practitioner Marketing — extracted to controllers/practitioner-marketing.js
// ── Practitioner Management — extracted to controllers/practitioner-management.js
// ── Practitioner Notes — extracted to controllers/practitioner-notes.js ───────
// ── Clusters ───────────────────────────────────────────────────
// ── Clusters — extracted to controllers/clustering-controller.js ─────────────
// ── Onboarding Story ───────────────────────────────────────────
...
// ── Lazy Controller Loader (GAP-001) ──────────────────────────
// Dynamically injects controller scripts under js/controllers/ on first use.
// Each promise is cached so the same file is never loaded twice.
const _ctrlLoading = {};
function _loadController(filename) {
  if (_ctrlLoading[filename]) return _ctrlLoading[filename];
  _ctrlLoading[filename] = new Promise(function (resolve, reject) {
    const s = document.createElement('script');
    s.src = 'js/controllers/' + filename;
    s.onload = resolve;
    s.onerror = function () {
      delete _ctrlLoading[filename]; // allow retry on next call
      const msg = '[Ctrl] Failed to load: ' + filename;
      if (window.DEBUG) console.error(msg);
      reject(new Error(msg));
    };
    document.head.appendChild(s);
  });
  return _ctrlLoading[filename];
}
...
// openPricingModal, openBillingPortal, setBillingPeriod → billing-controller.js (eager defer)
...
// saveAIContext → practitioner-notes.js (lazy)
...
// loadPractitionerMessages, sendPractitionerMessage, loadClientMessages, sendClientMessage
// → practitioner-clients.js (lazy)
...
// loadPractitionerInvitations, revokePractitionerInvitation → practitioner-clients.js (lazy)
// showNewNoteForm, hideNewNoteForm, saveSessionNote, editSessionNote, cancelEditNote,
// updateSessionNote, deleteSessionNote, loadMoreNotes, toggleDirectoryForm,
// saveDirectoryProfile, checkNotionStatus, connectNotion, syncNotionClients,
...
// ── Celebrity / Achievements — extracted to controllers/achievements-controller.js ──
