...
| **GTM & Growth** | 0 | ✅ 4 + ❌ 2 | ✅ 2 + ❌ 6 | ❌ 3 | 17 | ❌ 11 open — activation + trial nurture + landing-tier gating resolved; remaining work is proof, attribution, promo validation, support, and post-purchase onboarding |
...
- ✅ **GTM-BL-11** `BL-GTM-P1-5` — Implement practitioner_activated analytics event (resolved 2026-03-22)
- ✅ **GTM-BL-12** `BL-GTM-P1-6` — Gate 1→Gate 2 drop-off email nudge (resolved 2026-03-22)
- ✅ **GTM-BL-13** `BL-GTM-P2-1` — Hide Individual tier from marketing surfaces (resolved 2026-03-22)
...
- ✅ **GTM-BL-17** `BL-GTM-P2-5` — Email nurture sequence setup (7 emails, 14-day trial arc) (resolved 2026-03-22)
...
| **BL-OPS-P2-4** | **Cloudflare metrics credentials were configured, but audit tooling still missed them due to env-name drift** — `.env.local` already contains the Cloudflare token and account ID, and `CF_API_TOKEN` is deployed. The remaining problem was script drift: `run-audit.js` did not load `.env.local`, and one integration path still looked for the misspelled `CLOUFLARE_ACCOUNT_ID`. Fix applied: env aliases normalized and audit runner now loads local env before metrics collection. Linked: [Issue WC-005](audits/issue-registry.json). | ✅ Fixed (2026-03-21) | 15 min | [WC Assessment 2026-03-21](audits/issue-registry.json) |
| **BL-OPS-P2-5** | **Google OAuth secrets not configured in Workers production environment — Google Sign-In non-functional** — `workers/src/handlers/oauthSocial.js` handler is complete and tested. `GOOGLE_CLIENT_ID` and `GOOGLE_CLIENT_SECRET` were previously flagged as missing, but the current Workers secret inventory shows both present. Linked: [Issue WC-006](audits/issue-registry.json). | ✅ Fixed (2026-03-21) | 30 min | [WC Assessment 2026-03-21](audits/issue-registry.json) |
| **BL-OPS-P2-6** | **Apple Sign-In secrets not configured in Workers production environment — Apple Sign-In non-functional** — `workers/src/handlers/oauthSocial.js` handler is complete. `APPLE_CLIENT_ID`, `APPLE_TEAM_ID`, `APPLE_KEY_ID`, and `APPLE_PRIVATE_KEY` were previously flagged as missing, but the current Workers secret inventory shows them present. Linked: [Issue WC-007](audits/issue-registry.json). | ✅ Fixed (2026-03-21) | 1 hr | [WC Assessment 2026-03-21](audits/issue-registry.json) |
| **BL-OPS-P2-7** | **TELNYX_API_KEY and TELNYX_PHONE_NUMBER not set as Workers secrets — SMS delivery non-functional in production** — `workers/src/handlers/sms.js` is fully implemented. This was previously flagged from stale audit data, but the current Workers secret inventory shows the Telnyx keys present, and the Telnyx integration test validates the API key successfully. Linked: [Issue WC-008](audits/issue-registry.json). | ✅ Fixed (2026-03-21) | 20 min | [WC Assessment 2026-03-21](audits/issue-registry.json) |
